...
public:
	...
	void d3() {
		...
		fprintf(help_stream,
			...
			"--focus <args>    Create focus region with <args>:\n"
			"                     <ci> <sd> <ed> <ap> <sc> <ft>:\n"
			"                         <ci>  camera index\n"
			"                         <sd>  start depth (at frame center)\n"
			"                         <ed>  end depth (at frame center)\n"
			"                         <ap>  aperture diameter\n"
			"                         <sc>  view sample count\n"
			"                         <ft>  focus type:\n"
			"                                 v <min> <max>   multi-focus within [min, max]\n"
			"                                 d <d>           focus at distance <d>\n"
			"                                 p <x> <y>       set focus from point (x, y)\n"
			"                                 dg <d> <h> <v>          \  focus with tilt\n"
			"                                 pg <x> <y> <h> <v>      /  gradients (h, v)\n"
			BETWEEN_SECTIONS
			"Camera parameters:\n"
			HEADER_SPACE
